"undefined" == typeof cyberplayer && (cyberplayer = function (d) {
	if (cyberplayer.api)
		return cyberplayer.api.selectPlayer(d)
	...
	b.saveCookie = function (b, a) {
		h.cookie = "cyberplayer." + b + "\x3d" + a + "; path\x3d/"
	};
	b.getCookies = function () {
		for (var b = {}, a = h.cookie.split("; "), e = 0; e < a.length; e++) {
			var c = a[e].split("\x3d");
			0 == c[0].indexOf("cyberplayer.") && (b[c[0].substring(9, c[0].length)] = c[1])
		}
		return b
	};
	b.typeOf = function (b) {
	...
	b.translateEventResponse = function (a, e) {
		var c = b.extend({}, e);
		a == d.events.CBPLAYER_FULLSCREEN && !c.fullscreen ? (c.fullscreen = "true" == c.message ? !0 : !1, delete c.message) : "object" == typeof c.data ? (c = b.extend(c, c.data), delete c.data) : "object" == typeof c.metadata && b.deepReplaceKeyName(c.metadata, ["__dot__", "__spc__", "__dsh__", "__default__"], [".", " ", "-", "default"]);
	...
	d.key = function (a) {
		...
		this.edition = function () {
			return "premium";//return c && c.getTime() < (new Date).getTime() ? "invalid" : h
		};
		this.token = function () {
			return g
		};
	...
	a.longsToStr = function (a) {
		for (var b = Array(a.length), e = 0; e < a.length; e++)
			b[e] = String.fromCharCode(a[e] & 255, a[e] >>> 8 & 255, a[e] >>> 16 & 255, a[e] >>> 24 & 255);
		return b.join("")
	};
	var h = {
			...
			if (c[b]) {
				try {
					for (var g = 0; g < c[b].length; g++)
						if (c[b][g].listener.toString() == d.toString()) {
							c[b].splice(g, 1);
							break
						}
				} catch (h) {
	...
	(d.embed.config = function (b) {
		var e = {
			...
			primary : "html5",
			width : 480,
			base : b.base ? b.base : g.getScriptPath("cyberplayer.js")
		};
		b = g.extend(e, d.defaults, b);
		var e = {
			type : "html5",
			src : b.base + "cyberplayer.html5.js"
		},
		c = {
			type : "flash",
	...
	d.embed.download = function (d, c, b) {
		...
		if (r && r.length) {
			...
			m ? (b = m, d && (m = k("a", "display", d), k("div", "icon", m), k("div", "logo", m), b && m.setAttribute("href", a.getAbsolutePath(b))), b = "#" + d.id + " .cbdownload", d.style.width = "", d.style.height = "", e(b + "display", {
		...
		function n(a, b, c) {
			return function () {
				try {
					...
					var e = document.getElementById(l.id).getPluginConfig("display");
	...
	e = d.api = function (a) {
		...
		function m(a, b) {
			p[a] || (p[a] = [], q(g.CBPLAYER_PLAYER_STATE, function (b) {
					var c = b.newstate;
					b = b.oldstate;
					if (c == a) {
						var e = p[c];
		...
		function j() {
			if (u) {
				for (var a = arguments[0], b = [], c = 1; c < arguments.length; c++)
					b.push(arguments[c]);
				if ("undefined" != typeof s && "function" == typeof s[a])
					switch (b.length) {
					case 4:
		...
		f.addButton = function (a, b, c, e) {
			try {
				y[e] = c,
				j("cbDockAddButton", a, b, "cyberplayer('" + f.id + "').callback('" + e + "')", e)
			} catch (d) {
				h.log("Could not add dock button" + d.message)
			}
		};
		f.removeButton = function (a) {
			j("cbDockRemoveButton", a)
		};
		...
		f.releaseState = function () {
			return j("cbReleaseState")
		};
		f.getDuration = function () {
			return j("cbGetDuration")
		};
		f.getFullscreen = function () {
			return j("cbGetFullscreen")
		};
		f.getStretching = function () {
			return j("cbGetStretching")
		};
		f.getHeight = function () {
			return j("cbGetHeight")
		};
		f.getLockState = function () {
			return j("cbGetLockState")
		};
		f.getMeta = function () {
		...
		f.getVolume = function () {
			return j("cbGetVolume")
		};
		f.getWidth = function () {
			return j("cbGetWidth")
		};
		f.setFullscreen = function (a) {
			h.exists(a) ? j("cbSetFullscreen", a) : j("cbSetFullscreen", !j("cbGetFullscreen"));
			return f
		};
		...
		f.setMute = function (a) {
			h.exists(a) ? j("cbSetMute", a) : j("cbSetMute", !j("cbGetMute"));
			return f
		};
		f.lock = function () {
			return f
		...
		f.play = function (a) {
			"undefined" == typeof a ? (a = f.getState(), a == c.PLAYING || a == c.BUFFERING ? j("cbPause") : j("cbPlay")) : j("cbPlay", a);
			return f
		};
		f.pause = function (a) {
			"undefined" == typeof a ? (a = f.getState(), a == c.PLAYING || a == c.BUFFERING ? j("cbPause") : j("cbPlay")) : j("cbPause", a);
			return f
		};
		f.stop = function () {
			j("cbStop");
		...
		f.getQualityLevels = function () {
			return j("cbGetQualityLevels")
		};
		f.getCurrentQuality = function () {
			return j("cbGetCurrentQuality")
		};
		f.setCurrentQuality = function (a) {
			j("cbSetCurrentQuality", a)
		};
		f.getCaptionsList = function () {
			return j("cbGetCaptionsList")
		};
		f.getCurrentCaptions = function () {
			return j("cbGetCurrentCaptions")
		};
		f.setCurrentCaptions = function (a) {
			j("cbSetCurrentCaptions", a)
		};
		f.getControls = function () {
			return j("cbGetControls")
		};
		f.getSafeRegion = function () {
			return j("cbGetSafeRegion")
		};
		...
		f.playerReady = function (a) {
			...
			q(g.CBPLAYER_PLAYLIST_ITEM, function () {
				x = {}

			});
			q(g.CBPLAYER_MEDIA_META, function (a) {
