...
(function (g) {
	...
	function f(a, b, c, f) {
		return function () {
			...
			(e = g.parseXML(a.responseText)) && e.firstChild ? (a = g.extend({}, a, {
						responseXML : e
					}), c(a)) : f && f(a.responseText ? "错误的XML格式" : b)
		}
	}
	var d = window;
	...
	function (a) {
		...
		if (a) {
			...
			for (var e = 0; e < a.length; e++) {
				var d = a[e].type,
				h = a[e].file;
				d || (d = f.extType(g.extension(h)), a[e].type = d);
				if ((h = g.extensionmap.types[d]) && cyberplayer.vid.canPlayType(h))
					b || (b = d), d == b && c.push(g.extend({}, a[e]))
			}
		}
		return c
	};
	g.ajax = function (a, b, c) {
		...
		if (e && g.exists(d.XDomainRequest))
			k = new XDomainRequest, k.onload = f(k, a, b, c), k.onerror =
				h(c, a, k);
	...
	g.seconds = function (a) {
		...
		var b = a.split(":"),
		c = 0;
		"s" == a.substr(-1) ? c = Number(a.substr(0, a.length - 1)) : "m" == a.substr(-1) ?
			c = 60 * Number(a.substr(0, a.length - 1)) : "h" == a.substr(-1) ? c = 3600 * Number(a.substr(0, a.length - 1)) : 1 < b.length ? (c = Number(b[b.length - 1]), c += 60 * Number(b[b.length - 2]), 3 == b.length && (c += 3600 * Number(b[b.length - 3]))) : c = Number(a);
		return c
	};
	g.bounds = function (a) {
		if (!a)
		...
		do
			c += isNaN(b.offsetLeft) ? 0 : b.offsetLeft, f += isNaN(b.offsetTop) ? 0 : b.offsetTop;
...
(function (g) {
	...
	g.transform = function (a, b) {
		...
		e;
		b = b ? b : "";
		"string" == typeof a ? (e = {}, e["-webkit" + c] = b, e["-ms" + c] = b, e["-moz" + c] = b, e["-o" + c] = b, g.css(a, e)) : (c = "Transform", e = a.style, e["webkit" + c] = b, e["Moz" + c] = b, e["ms" + c] = b, e["O" + c] = b)
	};
	...
	j(".cyberplayer " + " div span a img ul li video".split(" ").join(",.cyberplayer ") + ", .cyberpclick", {
		margin : 0,
		padding : 0,
		border : 0,
		color : "#000000",
...
(function (g) {
	...
	g.stretch = function (f, d, a, b, c, k) {
		if (d && (f || (f = h.UNIFORM), a && b && c && k)) {
			var e = a / c,
			m = b / k,
			l = "video" == d.tagName.toLowerCase(),
			j = !1,
			r;
			l && g.transform(d);
			r = "cyberp" + f.toLowerCase();
			...
			case h.EXACTFIT:
				...
				break;
			default:
				e > m ? 0.95 < c * m / a ? (j = !0, r =
						"cyberpexactfit") : (c *= m, k *= m) : 0.95 < k * e / b ? (j = !0, r = "cyberpexactfit") : (c *= e, k *= e),
				j && (m = Math.ceil(100 * b / k) / 100, e = Math.ceil(100 * a / c) / 100)
			}
			l ? j ? (d.style.width = c + "px", d.style.height = k + "px", g.scale(d, e, m, (a - c) / 2 / e, (b - k) / 2 / m)) : (d.style.width = "", d.style.height = "") : (d.className = d.className.replace(/\s*cyberp(none|exactfit|uniform|fill)/g, ""), d.className += " " + r)
		}
	};
	var h = g.stretching = {
		NONE : "none",
			...
			if (4 === d.readyState)
				if (200 === d.status) {
					...
					for (m = 0; m < c.length; m++) {
						...
						e = l;
						e.text && (k.push(e), e.end && (k.push({
									begin : e.end,
									text : ""
								}), delete e.end))
					}
					1 < k.length ? h(k) : f("错误的DFXP文件格式: " + a)
				} else
					k = d.status, 0 == k ? f("Crossdomain loading denied: " + a) : 404 == k ? f("DFXP文件不存在: " + a) : f("错误 " + k + " 当载入DFXP时: " +	a)
		}
	}
})(cyberplayer.html5.parsers);
(function (g) {
	var h = g.html5.parsers;
	(h.cyberpparser = function () {}).parseEntry = function (f, d) {
		for (var a = [], b = [], c = g.utils.xmlAttribute, k = 0; k < f.childNodes.length; k++) {
			var e = f.childNodes[k];
			if ("cyberplayer" == e.prefix) {
				var m = h.localName(e);
				"source" == m ? (delete d.sources, a.push({
						file : c(e, "file"),
						"default" : c(e, "default"),
						label : c(e, "label"),
						type : c(e, "type")
					})) : "track" == m ? (delete d.tracks, b.push({
						file : c(e, "file"),
						"default" : c(e, "default"),
						kind : c(e, "kind"),
						label : c(e, "label")
					})) : (d[m] = g.utils.serialize(h.textContent(e)), "file" ==
					m && d.sources && delete d.sources)
			}
...
(function (g) {
	function h(b) {
		...
		a = g.cyberpparser.parseEntry(b, a);
		return new cyberplayer.playlist.item(a)
	}
	var f =
		cyberplayer.utils,
	d = g.textContent,
...
(function (g) {
	g.srt = function (h, f, d) {
		function a(a) {
			0 == a ? f("跨域载入拒绝: " + k) : 404 == a ? f("SRT文件不存在: " + k) : f("错误 " + a + " 当载入SRT文件时: " + k)
		}
		function b(a) {
			var b = d ? [] : [{
						begin : 0,
			...
			for (a = 0; a < c.length; a++)
				if ("WEBVTT" != c[a]) {
					...
					try {
						...
						0 < y && (e.begin =
								g(t[n].substr(0, y)), e.end = g(t[n].substr(y + 5)));
						if (t[n + 1]) {
							e.text = t[n + 1];
					...
					e.text && (b.push(e), e.end && !d && (b.push({
								begin : e.end,
								text : ""
							}), delete e.end))
				}
		...
		this.load = function (d) {
			...
			try {
				...
				h && e.exists(window.XDomainRequest) && (c = new XDomainRequest, c.onload = function () {
					b(c.responseText)
				},
					c.onerror = function () {
					a(c.status)
...
(function (g) {
	...
	g.captions = function (a, e) {
		...
		function l(a) {
			(I = a.fullscreen) ? (j(), setTimeout(j, 500)) : t(!0)
		}
		function j() {
		...
		function r(a) {
			("tt" == g.parsers.localName(a.responseXML.firstChild) ? new cyberplayer.html5.parsers.dfxp(n, m) : new cyberplayer.html5.parsers.srt(n, m)).load(B)
		}
		function q() {
			(new cyberplayer.html5.parsers.srt(n,
					m)).load(B)
		}
		function n(a) {
			v.populate(a);
		...
		function t(a) {
			h.isMobile() || !A.length ? v.hide() : C == b && 0 < E ? (v.show(), I ? l({
					fullscreen : !0
				}) : (y(), a && setTimeout(y, 500))) : v.hide()
		}
		function y() {
			var a = w.cbGetWidth();
			w._model && (w._model.config && w._model.config.listbar && w._model.config.listbar.size) && (a -= w._model.config.listbar.size);
			v.resize(a, Math.round(0.94 * w.cbGetHeight()))
		}
		function p(a) {
			0 < a ? (D = a - 1, E = a) : E = 0;
			D >= A.length || (A[D].data ? v.populate(A[D].data) : (B = a = A[D].file, h.ajax(a, r, q)),
				t(!1))
		}
		function z() {
			var a = [];
		...
		this.setCurrentCaptions = function (a) {
			if (0 <= a && E != a && a <= A.length) {
				...
				var b = f.CBPLAYER_CAPTIONS_CHANGED;
				J.sendEvent(b, {
					type : b,
					tracks : a,
					track : E
		...
		s.id = w.id + "_caption";
		s.className = "cyberpcaptions";
		w.cbAddEventListener(f.CBPLAYER_PLAYER_STATE, function (a) {
			switch (a.newstate) {
			case d.IDLE:
				C = "idle";
				t(!1);
		...
		w.cbAddEventListener(f.CBPLAYER_PLAYLIST_ITEM, function () {
			D = 0;
			A = [];
			v.update(0);
			for (var a = w.cbGetPlaylist()[w.cbGetPlaylistIndex()].tracks, b = [], c = 0, e = "", d = 0, e = "", c = 0; c < a.length; c++)
				e =
					a[c].kind.toLowerCase(), ("captions" == e || "subtitles" == e) && b.push(a[c]);
			for (c = E = 0; c < b.length; c++)
				if (e = b[c].file)
					b[c].label || (b[c].label = c.toString()), A.push(b[c]);
		...
		w.cbAddEventListener(f.CBPLAYER_MEDIA_ERROR, m);
		w.cbAddEventListener(f.CBPLAYER_ERROR, m);
		w.cbAddEventListener(f.CBPLAYER_READY,
			function () {
			for (var a in u)
				e && void 0 != e[a.toLowerCase()] ? "color" == a ? x.color = "#" + String(e.color).substr(-6) : x[a] = e[a.toLowerCase()] : x[a] = u[a];
			v = new cyberplayer.html5.captions.renderer(x, s);
			t(!1)
		});
		w.cbAddEventListener(f.CBPLAYER_MEDIA_TIME, function (a) {
			v.update(a.position)
		});
		w.cbAddEventListener(f.CBPLAYER_FULLSCREEN, l)
	};
	a(".cyberpcaptions", {
		position : "absolute",
		cursor : "pointer",
	...
	g.captions.renderer = function (h, f) {
		...
		function b() {
			for (var a = -1, b = 0; b < e.length; b++)
				if (e[b].begin <= j && (b ==
						e.length - 1 || e[b + 1].begin >= j)) {
					a = b;
					break
				}
			-1 == a ? d("") : a != l && (l = a, d(e[b].text))
		}
		function c(a) {
			for (var b in a)
				g.style[b] = a[b]
...
(function (g) {
	...
	h.controlbar = function (g, s) {
		...
		function x(a) {
			var b = q,
			c;
			K.elapsed && (c = f.timeFormat(a.position), K.elapsed.innerHTML = c, b = c.length != f.timeFormat(ra).length);
			K.duration && (c = f.timeFormat(a.duration), K.duration.innerHTML = c, b = b || c.length != f.timeFormat(na).length);
		...
		function E(a) {
			ha && (Ha = a.track, ia && 0 <= Ha && ia.setActive(a.track))
		}
		function I() {
		...
		function O(a) {
			...
			case e:
				var d;
				a.width ? (d = J(), d.className = "cyberpblankDivider", b(d, {
						width : parseInt(a.width)
					})) : a.element ? d = F(a.element) : (d = F(a.name), d || (d = J(), d.className = "cyberpblankDivider"));
			...
			case k:
				...
				if (f.src) {
					var g = J();
					g.id = Z + "_" + a;
					g.className = "cyberptext cyberp" + a;
			...
			case c:
				if ("blank" != a.name)
					return a = a.name, H(a + "Button").src ? (d = J(), d.className = "cyberp" + a + " cyberpbuttoncontainer", f = z.createElement("button"), f.addEventListener("click", U(a), q), f.innerHTML = "\x26nbsp;", d.appendChild(f),
						f = H(a + "Button"), g = H(a + "ButtonOver"), M(B(".cyberp" + a + " button"), f, g), (f = bb[a]) && M(B(".cyberp" + a + ".cyberptoggle button"), H(f + "Button"), H(f + "ButtonOver")), a = K[a] = d) : a = t, a;
				break;
			case m:
				f = a.name;
				a = J();
				...
				r = F(n + E, t, q, q, g);
				d = J();
				var u = ["Rail", "Buffer", "Progress"],
				A;
				d.className = "cyberprail cyberpsmooth";
				for (var p = 0; p < u.length; p++) {
					...
					if (x) {
						var O = J();
						O.className = "cyberprailgroup " + u[p];
						D && O.appendChild(D);
						O.appendChild(x);
						I && (O.appendChild(I), I.className += " cyberpcap" + (g ? "Bottom" : "Right"));
						b(B(".cyberprailgroup." + u[p]), {
				...
				if (v = F(f + v + "Thumb", t, q, q, g))
					...
					}), v.className += " cyberpthumb", (g && A ? A : d).appendChild(v);
				d.addEventListener("mousedown", pa(f), q);
				"time" == f && (d.addEventListener("mousemove", R, q), d.addEventListener("mouseout", L, q));
				K[f + "Rail"] = d;
				A = H(n + j);
				n = H(n + j);
				H(f + "SliderRail");
				a.className = "cyberpslider cyberp" + f;
				s && a.appendChild(s);
				a.appendChild(d);
				r && (g && (r.className += " cyberpcapBottom"), a.appendChild(r));
				b(B(".cyberp" + f + " .cyberprail"), {
					left : g ? y : A.width,
					right : g ? y : n.width,
					top : g ? A.height : y,
					bottom : g ? n.height : y,
				...
				K[f] = a;
				a.vertical = g;
				"time" == f ? (ja = new h.overlay(Z + "_timetooltip", ba), Ia = new h.thumbs(Z + "_thumb"), Sa = z.createElement("div"), Sa.className = "cyberpoverlaytext", Ja = z.createElement("div"), f = Ia.element(), Ja.appendChild(f), Ja.appendChild(Sa), ja.setContents(Ja), Va = d, W(0), f = ja.element(), d.appendChild(f), K.timeSliderThumb && b(B(".cyberptimeSliderThumb"), {
						"margin-left" : H("timeSliderThumb").width / -2
					}), fa(0), T(0), T(0), fa(0)) : "volume" == f && (b(B(".cyberpvolume"), {
						width : H("volumeRail").width + (g ? 0 : H("volumeCap" +
								j).width + H("volumeCap" + E).width),
					...
					}), g && (a.className += " cyberpvertical"));
				return a
			}
		}
		function F(a, c, d, e, g) {
			var h = J();
			h.className = "cyberp" + a;
		...
		function ca() {
			za || (Ka.show(), V("fullscreen"))
		}
		function $(a, b) {
			f.exists(b) || (b = !La[a]);
			K[a] && (K[a].className = "cyberp" + a + (b ? " cyberptoggle cyberptoggling" :
						" cyberptoggling"), setTimeout(function () {
					K[a].className = K[a].className.replace(" cyberptoggling", "")
				}, 100));
			La[a] = b
		}
		function ka() {
			la && 1 < la.length && (Ta && (clearTimeout(Ta), Ta = void 0), ga.show(), V("hd"))
		}
		function Ma() {
			ha && 1 < ha.length && (Ua && (clearTimeout(Ua), Ua = void 0), ia.show(), V("cc"))
		}
		function Ea(a) {
			0 <= a && a < la.length && (G.cbSetCurrentQuality(a), ga.hide())
		}
		function Na(a) {
			0 <= a && a < ha.length && (G.cbSetCurrentCaptions(a), ia.hide())
		}
		function Oa(a) {
			a.preventDefault();
		...
		function pa(b) {
			return function (c) {
				0 ==
				c.button && (K[b + "Rail"].className = "cyberprail", "time" == b ? G.cbGetState() != a.IDLE && (G.cbSeekDrag(n), qa = b) : qa = b)
			}
		}
		function ua(a) {
			...
			if (50 < b - Xa) {
				if ((Pa = f.bounds(Va)) && 0 != Pa.width) {
					...
					0 <= d && d <= Pa.width && (c.style.left = Math.round(d) + "px", W(na * d / Pa.width), va = f.bounds(S))
				}
				Xa = b
			}
			if (qa && 0 == a.button)
				return c = K[qa].getElementsByClassName("cyberprail")[0], d = f.bounds(c), c = qa, d = K[c].vertical ? (d.bottom - a.pageY) / d.height : (a.pageX - d.left) / d.width, "mouseup" ==
				a.type ? ("time" == c && G.cbSeekDrag(q), K[c + "Rail"].className = "cyberprail cyberpsmooth", qa = t, Ya[c](d)) : ("time" == qa ? T(d) : Q(d), 500 < b - Za && (Za = b, Ya[qa](d))), !1
		}
		function R() {
			ja && (na && !za) && (X(ja), ja.show())
		}
		function L() {
			ja && ja.hide()
		}
		...
		function xa(a, c, d, e) {
			...
			e ? c.addEventListener("mouseout",
				e, q) : c.addEventListener("mouseout", a.hide, q);
			b("#" + f.id, {
				left : "50%"
			})
		}
		function ma(a) {
			var b = J();
			b.className = "cyberpgroup cyberp" + a;
			Ba[a] = b;
			if (Y[a] && (b = Y[a], a = Ba[a], b && 0 < b.elements.length))
				for (var c = 0; c < b.elements.length; c++) {
					var d = O(b.elements[c]);
					d && ("volume" == b.elements[c].name && d.vertical ? (Aa = new h.overlay(Z + "_volumeOverlay", ba), Aa.setContents(d)) : a.appendChild(d))
				}
		}
		function Qa() {
			1 < G.cbGetPlaylist().length && (!z.querySelector("#" + G.id + " .cyberpplaylist") || G.cbGetFullscreen()) ? (b(B(".cyberpnext"), r), b(B(".cyberpprev"),
					r), b(B(".nextdiv"), r)) : (b(B(".cyberpnext"), j), b(B(".cyberpprev"), j), b(B(".nextdiv"), j))
		}
		function X(a) {
			va || (va = f.bounds(S));
			a.offsetX(0);
		...
		function Ca(a, b) {
			...
			d = K[d + "Thumb"],
			g = q;
			f && (c ? (f.style.height = e, f.style.bottom = 0, f.clientHeight <= f.capSize && (g = n)) : (f.style.width = e, f.clientWidth <= f.capSize && (g = n)), f.style.opacity = !g && 0 < b || qa ? 1 : 0);
		...
		ea.redraw = function () {
			...
			var a = H("capLeft"),
			c = H("capRight");
			b(B(".cyberpgroup.cyberpcenter"), {
				left : Math.round(f.parseDimension(Ba.left.offsetWidth) +
					a.width),
		...
		ea.audioMode = function (a) {
			a != za && (za = a, b(B(".cyberpfullscreen"), {
					display : a ? "none" : y
				}), b(B(".cyberphd"), {
					display : a ? "none" : y
				}), b(B(".cyberpcc"), {
		...
		ea.show = function () {
			ea.visible || (clearTimeout(Fa), Fa = y, ea.visible = !0, S.style.display = "inline-block", Da(), v(), Fa = setTimeout(function () {
						S.style.opacity = 1
					}, 10))
		};
		ea.hide = function () {
			ea.visible && (ea.visible = !1, S.style.opacity = 0, clearTimeout(Fa), Fa = y, Fa = setTimeout(function () {
		...
		K.hd && (ga = new h.menu("hd", Z + "_hd", ba, Ea), xa(ga, K.hd, ka, da), ta.hd = ga);
		K.cc && (ia = new h.menu("cc", Z + "_cc", ba, Na), xa(ia, K.cc, Ma, wa), ta.cc = ia);
		K.mute && (K.volume && K.volume.vertical) && (Aa = new h.overlay(Z + "_volumeoverlay",
					ba), Aa.setContents(K.volume), xa(Aa, K.mute, N), ta.volume = Aa);
		K.fullscreen && (Ka = new h.overlay(Z + "_fullscreenoverlay", ba), Ga = z.createElement("div"), Ga.className = "cyberpoverlaytext", Ga.innerHTML = "Fullscreen", Ka.setContents(Ga), xa(Ka, K.fullscreen, ca), ta.fullscreen = Ka);
		b(B(".cyberpright"), {
			right : H("capRight").width
		});
		aa && S.appendChild(aa);
		G.cbAddEventListener(d.CBPLAYER_MEDIA_TIME, x);
		G.cbAddEventListener(d.CBPLAYER_PLAYER_STATE, function (c) {
			switch (c.newstate) {
			case a.BUFFERING:
			case a.PLAYING:
				b(B(".cyberptimeSliderThumb"), {
					opacity : 1
				});
				$("play", n);
				break;
			...
			case a.IDLE:
				...
				K.timeRail && (K.timeRail.className = "cyberprail", setTimeout(function () {
						K.timeRail.className += " cyberpsmooth"
					}, 100)),
				fa(0),
				x({
					position : 0,
		...
		G.cbAddEventListener(d.CBPLAYER_PLAYLIST_ITEM, function (a) {
			a = G.cbGetPlaylist()[a.index].tracks;
			if ("array" == f.typeOf(a))
				for (var b = 0; b < a.length; b++)
		...
		G.cbAddEventListener(d.CBPLAYER_MEDIA_MUTE, v);
		G.cbAddEventListener(d.CBPLAYER_MEDIA_VOLUME, C);
		G.cbAddEventListener(d.CBPLAYER_MEDIA_BUFFER, function (a) {
			fa(a.bufferPercent / 100)
		});
		G.cbAddEventListener(d.CBPLAYER_FULLSCREEN, function (a) {
			$("fullscreen", a.fullscreen);
			Qa()
		});
		G.cbAddEventListener(d.CBPLAYER_PLAYLIST_LOADED, D);
		G.cbAddEventListener(d.CBPLAYER_MEDIA_LEVELS, function (a) {
		...
		G.cbAddEventListener(d.CBPLAYER_MEDIA_LEVEL_CHANGED, A);
		G.cbAddEventListener(d.CBPLAYER_CAPTIONS_LIST, function (a) {
		...
		G.cbAddEventListener(d.CBPLAYER_CAPTIONS_CHANGED, E);
	...
	b(".cyberpcontrolbar span", {
		height : l
	});
	f.dragStyle(".cyberpcontrolbar span", "none");
	b(".cyberpcontrolbar .cyberpgroup", {
		display : "inline"
	});
	b(".cyberpcontrolbar span, .cyberpcontrolbar .cyberpgroup button,.cyberpcontrolbar .cyberpleft", {
		position : "relative",
		"float" : "left"
	});
	b(".cyberpcontrolbar .cyberpright", {
		position : "absolute"
	});
	b(".cyberpcontrolbar .cyberpcenter", {
		position : "absolute"
	});
	b(".cyberpcontrolbar buttoncontainer,.cyberpcontrolbar button", {
		display : "inline-block",
		height : l,
		border : "none",
		cursor : "pointer"
	});
	b(".cyberpcontrolbar .cyberpcapRight,.cyberpcontrolbar .cyberptimeSliderCapRight,.cyberpcontrolbar .cyberpvolumeCapRight", {
		right : 0,
		position : "absolute"
	});
	b(".cyberpcontrolbar .cyberpcapBottom", {
		bottom : 0,
		position : "absolute"
	});
	b(".cyberpcontrolbar .cyberptime", {
		position : "absolute",
		height : l,
		width : l,
		left : 0
	});
	b(".cyberpcontrolbar .cyberpthumb", {
		position : "absolute",
		height : l,
		cursor : "pointer"
	});
	b(".cyberpcontrolbar .cyberprail", {
		position : "absolute",
		cursor : "pointer"
	});
	b(".cyberpcontrolbar .cyberprailgroup", {
		position : "absolute",
		width : l
	});
	b(".cyberpcontrolbar .cyberprailgroup span", {
		position : "absolute"
	});
	b(".cyberpcontrolbar .cyberpdivider+.cyberpdivider", {
		display : "none"
	});
	b(".cyberpcontrolbar .cyberptext", {
		padding : "0 5px",
		"text-align" : "center"
	});
	b(".cyberpcontrolbar .cyberpoverlaytext", {
		padding : 3,
		"text-align" : "center"
	});
	b(".cyberpcontrolbar .cyberpvertical *", {
		display : "block"
	});
	g(".cyberpcontrolbar", "opacity .15s, background .15s, visibility .15s");
	g(".cyberpcontrolbar button", "opacity .15s, background .15s, visibility .15s");
	g(".cyberpcontrolbar .cyberptime .cyberpsmooth span", "opacity .15s, background .15s, visibility .15s, width .15s linear, left .05s linear");
	g(".cyberpcontrolbar .cyberptoggling", "none")
})(cyberplayer);
(function (g) {
	var h = g.html5,
	f = g.utils,
	...
	h.controller = function (b, c) {
		...
		function e(a) {
			...
			case "string":
				...
				b.addEventListener(d.CBPLAYER_PLAYLIST_LOADED, function (a) {
					e(a.playlist)
				});
				b.addEventListener(d.CBPLAYER_ERROR, function (a) {
					e([]);
		...
		function m(b) {
			...
			try {
				0 <= u && (e(u), u = -1);
				v = m;
				if (!x && (x = !0, w.sendEvent(d.CBPLAYER_MEDIA_BEFOREPLAY), x = !1, D)) {
					D = !1;
					v = null;
					return
				}
				...
				w.sendEvent(d.CBPLAYER_ERROR, c),
				v = null
			}
			return !1
		}
		...
		function n() {
			p.state == a.IDLE && (C ? C = !1 : (v = n, p.repeat ? q() : p.item == p.playlist.length - 1 ? (u = 0, l(!0), setTimeout(function () {
								w.sendEvent(d.CBPLAYER_PLAYLIST_COMPLETE)
							}, 0)) : q()))
		}
		function t(a) {
			return function () {
		...
		function (a) {
			if (!s) {
				c.completeSetup();
				w.sendEvent(a.type, a);
				g.utils.exists(window.cyberplayer.playerReady) && g.playerReady(a);
				p.addGlobalListener(k);
				c.addGlobalListener(k);
				w.sendEvent(g.events.CBPLAYER_PLAYLIST_LOADED, {
					playlist : p.playlist
				});
				w.sendEvent(g.events.CBPLAYER_PLAYLIST_ITEM, {
					index : p.item
				});
				e();
				p.autostart && !f.isMobile() && m();
				for (s = !0; 0 < A.length; )
					a = A.shift(), y(a.method, a.arguments)
			}
		};
		p.addEventListener(d.CBPLAYER_MEDIA_BUFFER_FULL, function () {
			z.play()
		});
		p.addEventListener(d.CBPLAYER_MEDIA_COMPLETE,
			function () {
			setTimeout(n, 25)
		});
		p.addEventListener(d.CBPLAYER_MEDIA_ERROR, function (a) {
			a = f.extend({}, a);
			a.type = d.CBPLAYER_ERROR;
			w.sendEvent(a.type, a)
		})
	}
})(cyberplayer);
...
(function (g) {
	...
	g.display = function (m, l) {
		function j() {
			...
			N.sendEvent(f.CBPLAYER_DISPLAY_CLICK)
		}
		function r(a, b) {
			V.showicons && (a || b ? (F.setRotation("buffer" == a ? parseInt(V.bufferrotation) : 0, parseInt(V.bufferinterval)), F.setIcon(a), F.setText(b)) : F.hide())
		}
		function q(b) {
			C != b ? (C && w(c, e), (C = b) ? (b = new Image, b.addEventListener("load",
						y, e), b.src = C) : (a("#" + u.id + " " + c, {
		...
		function t(a) {
			a = M ? M : s ? s.cbGetState() : d.IDLE;
			if (a != U)
				switch (U = a, F && F.setRotation(0), a) {
				case d.IDLE:
					!B && !J && (C && !E && w(c, k), r("play", v ? v.title : ""));
					break;
				case d.BUFFERING:
					B = e;
		...
		function z() {
			0 < u.clientWidth * u.clientHeight && h.stretch(s.cbGetStretching(), x, u.clientWidth, u.clientHeight, D, A)
		}
		function w(b, c) {
			h.exists(O[b]) || (O[b] = !1);
			O[b] != c && (O[b] = c, a("#" + u.id + " " + b, {
					opacity : c ? 1 : 0,
		...
		V = h.extend({
				...
				m.skin.getComponentSettings("display"), l),
		N = new f.eventdispatcher;
		h.extend(this, N);
		this.clickHandler = j;
		var ca;
		...
		this.redraw = z;
		this.show = function () {
			F && (M ? M : s ? s.cbGetState() : d.IDLE) != d.PLAYING && F.show()
		};
		this.hide = function () {
			F && F.hide()
		};
		...
		u.id = s.id + "_display";
		u.className = "cyberpdisplay";
		x = b.createElement("div");
		x.className = "cyberppreview cyberp" + s.cbGetStretching();
		u.appendChild(x);
		s.cbAddEventListener(f.CBPLAYER_PLAYER_STATE, n);
		s.cbAddEventListener(f.CBPLAYER_PLAYLIST_ITEM, function () {
			B = e;
			I.error && I.error.setText();
			var a = (v = s.cbGetPlaylist()[s.cbGetPlaylistIndex()]) ? v.image : "";
			U = void 0;
			q(a)
		});
		s.cbAddEventListener(f.CBPLAYER_PLAYLIST_COMPLETE, function () {
			J = k;
			r("replay");
			var a = s.cbGetPlaylist()[0];
			q(a.image)
		});
		s.cbAddEventListener(f.CBPLAYER_MEDIA_ERROR,
			p);
		s.cbAddEventListener(f.CBPLAYER_ERROR, p);
...
(function (g) {
	...
	g.displayicon = function (g, e, m, l) {
		...
		function q(a, b, c, d) {
			...
			e.src && (c["background-image"] = "url(" + e.src + ")", c.width = e.width);
			f(j(b), c);
			d = h.extend({}, d);
			e.overSrc &&
			(d["background-image"] = "url(" + e.overSrc + ")");
			A = e;
			f("#" + p.id + " .cyberpdisplay:hover " + (b ? b : j()), d)
		}
		function n(a) {
		...
		f("#" + p.id + " .cyberpdisplay:hover " + j(), O);
		C = r("cyberptext", s, m, l);
		D = r("icon", s);
		e();
		t()
	};
	...
	f(".cyberpdisplayIcon .cyberptext", {
		color : "#fff",
		padding : "0 1px",
		"max-width" : "300px",
		"overflow-y" : "hidden",
...
(function (g) {
	...
	g.dock = function (e, m) {
		...
		v.hide = function () {
			v.visible && (v.visible = !1, u.style.opacity = 0, setTimeout(function () {
					u.style.display = c
				}, 150))
		};
		v.show = function () {
			!v.visible && z && (v.visible = !0, u.style.display = k, setTimeout(function () {
					u.style.opacity = 1
				}, 0))
		};
		...
		v.removeButton = function (a) {
			w[a] && (x.removeChild(w[a].element), x.removeChild(w[a].divider), delete w[a], z--, n())
		};
		...
		C && (f(j(), {
			...
			}), l("capLeft", x), l("capRight", x), l("divider"));
		setTimeout(function () {
			d(u)
		})
	};
	...
	f(".cyberpdock \x3e .cyberpoverlay", {
		height : "auto",
		"float" : c,
		"z-index" : 99
	});
	...
	h.transitionStyle(".cyberpdock", "background .15s, opacity .15s");
	h.transitionStyle(".cyberpdock .cyberpoverlay", "opacity .15s");
...
(function (g) {
	...
	g.instream = function (c, h, e, m) {
		...
		this.load = function (e, F) {
			...
			w = m.detachMedia();
			v || (v = new g.video(w), v.addGlobalListener(l), v.addEventListener(d.CBPLAYER_MEDIA_META, q), v.addEventListener(d.CBPLAYER_MEDIA_COMPLETE, r), v.addEventListener(d.CBPLAYER_MEDIA_BUFFER_FULL, j));
			v.attachMedia();
			v.mute(h.mute);
			v.volume(h.volume);
			B = new g.model({}, v);
			...
			x = h.playlist[h.item];
			u = c.cbGetState();
			u == a.BUFFERING || u == a.PLAYING ? w.pause() : u == a.IDLE &&
			c.cbStop();
			s = w.currentTime;
			D = new g.display(J);
			D.setAlternateClickHandler(function (b) {
				B.state == a.PAUSED ? J.cbInstreamPlay() : A && E.sendEvent(d.CBPLAYER_INSTREAM_CLICK, b)
			});
			I.appendChild(D.element());
			f.isMobile() || (C = new g.controlbar(J), I.appendChild(C.element()), C.show());
			z.setupInstream(I, w);
			n();
			v.load(B.playlist[0])
		};
		this.cbInstreamDestroy = function (b) {
			if (A) {
				A = !1;
				v.load(x, !1);
				u == a.IDLE && v.stop(!0);
				...
				E.sendEvent(d.CBPLAYER_INSTREAM_DESTROYED, {
					reason : b ? "complete" : "destroyed"
				});
				m.attachMedia();
				if (u == a.BUFFERING || u == a.PLAYING)
					w.play(), h.playlist[h.item] == x && h.getVideo().seek(s)
			}
		};
		...
		this.cbPlay = function () {
			"true" == p.controlbarpausable.toString().toLowerCase() && this.cbInstreamPlay()
		};
		this.cbPause = function () {
			"true" == p.controlbarpausable.toString().toLowerCase() && this.cbInstreamPause()
		};
		this.cbStop = function () {
			"true" ==
			p.controlbarstoppable.toString().toLowerCase() && (this.cbInstreamDestroy(), c.cbStop())
		};
		this.cbSeek = function (a) {
			...
			case "always":
				...
				break;
			case "backwards":
				B.position > a && this.cbInstreamSeek(a)
			}
		};
		this.cbSeekDrag = function (a) {
			B.seekDrag(a)
		};
		this.cbGetPosition = function () {};
		this.cbGetDuration = function () {};
		this.cbGetWidth = c.cbGetWidth;
		this.cbGetHeight = c.cbGetHeight;
		this.cbGetFullscreen = c.cbGetFullscreen;
		this.cbSetFullscreen = c.cbSetFullscreen;
		this.cbGetVolume =
		function () {
			return h.volume
		};
		...
		this.cbSetCurrentQuality = function () {};
		this.cbGetQualityLevels = function () {
			return []
		};
		this.skin = c.skin;
		this.id = c.id + "_instream";
		E = new d.eventdispatcher;
		c.cbAddEventListener(d.CBPLAYER_RESIZE, n);
		c.cbAddEventListener(d.CBPLAYER_FULLSCREEN, n);
		return this
	}
})(cyberplayer.html5);
(function (g) {
	...
	a = g.html5.logo = function (b, c) {
		function k(a) {
			...
			if (!q || !l.link)
				e.cbGetState() == d.IDLE || e.cbGetState() == d.PAUSED ? e.cbPlay() : e.cbPause();
			q && l.link && (e.cbPause(), e.cbSetFullscreen(!1), window.open(l.link, l.linktarget))
		}
		var e = b,
		m = e.id + "_logo",
		l,
		...
		this.hide = function (a) {
			if (l.hide || a)
				q = !1, j.style.visibility = "hidden", j.style.opacity = 0
		};
		this.show = function () {
			q = !0;
		...
		var n = "o";
		e.edition && (n = e.edition(), n = "pro" == n ? "p" : "premium" == n ? "r" : "ads" == n ? "a" : "free" == n ? "f" : "o");
		if ("o" == n || "f" == n)
			r.link = "http://www.baidu.com/cbpabout/?a\x3dl\x26v\x3d" + g.version + "\x26m\x3dh\x26e\x3d" + n;
		l = h.extend({}, r, c);
		l.hide = "true" == l.hide.toString();
		j = document.createElement("img");
		j.className = "cyberplogo";
		j.id = m;
		if (l.file) {
			var r = /(\w+)-(\w+)/.exec(l.position),
			n = {},
			t = l.margin;
			3 == r.length ? (n[r[1]] = t, n[r[2]] = t) : n.top = n.right = t;
			f("#" + m + " ", n);
	...
	h.transitionStyle(".cyberplogo", "visibility .15s, opacity .15s")
})(cyberplayer);
(function (g) {
	var h = g.html5,
	f = g.utils,
	...
	d("." + "cyberpmenu cyberpoption".replace(/ /g, " ."), {
		"background-repeat" : "no-repeat",
		cursor : "pointer",
		position : "relative"
	})
...
(function (g) {
	...
	g.model = function (d, a) {
		...
		r = {};
		r[f.CBPLAYER_MEDIA_MUTE] = "mute";
		r[f.CBPLAYER_MEDIA_VOLUME] = "volume";
		r[f.CBPLAYER_PLAYER_STATE] = "newstate-\x3estate";
		r[f.CBPLAYER_MEDIA_BUFFER] = "bufferPercent-\x3ebuffer";
		r[f.CBPLAYER_MEDIA_TIME] = "position,duration";
		c.getVideo = function () {
			return k
		};
		...
		c.setFullscreen = function (a) {
			a != c.fullscreen && (c.fullscreen = a, c.sendEvent(f.CBPLAYER_FULLSCREEN, {
					fullscreen : a
				}))
		};
		c.setPlaylist = function (a) {
			...
			b.playlist = d;
			0 == c.playlist.length ? c.sendEvent(f.CBPLAYER_ERROR, {
				message : "Error loading playlist: No playable sources found"
			}) : (c.sendEvent(f.CBPLAYER_PLAYLIST_LOADED, {
					playlist : a
				}), c.item = -1, c.setItem(0))
		};
		c.setItem = function (a) {
			a = a == c.playlist.length || -1 > a ? 0 : -1 == a || a > c.playlist.length ? c.playlist.length -
				1 : a;
			a != c.item && (c.item = a, c.sendEvent(f.CBPLAYER_PLAYLIST_ITEM, {
					index : c.item
				}))
		};
		...
		c.playlist = [];
		c.setItem(0);
		a ? (k = a, e = k.getTag()) : (e = document.createElement("video"), k = new g.video(e));
		k.volume(c.volume);
		k.mute(c.mute);
...
(function (g) {
	...
	g.html5.overlay = function (d, g, q) {
		...
		function p(d, g) {
			g || (g = "");
			var h = y("cap" + d + g, "cyberpborder cyberp" + d + (g ? g : "")),
			j = h[0],
			h = h[1],
			l = {
			...
			g && (j[g] = h[g], j[d] = 0, l[d] = h[d], l[g] = 0, f(n("cyberp" + d), j), f(n("cyberp" + g), l), E[d] = h[d], E[g] = h[g])
		}
		function z() {
			0 !=
			u.clientWidth && (f(n(), {
		...
		u = t(".cyberpoverlay".replace(".", ""));
		u.id = s;
		C = y("arrow", "cyberparrow")[1];
		D = C.height;
		f(n("cyberparrow"), {
			position : "absolute",
			bottom : A ? e : 0,
	...
	f(".cyberpoverlay .cyberpcontents", {
		position : "relative",
		"z-index" : 1
	});
	f(".cyberpoverlay .cyberpborder", {
		position : "absolute",
		"background-size" : "100% 100%"
	}, !0);
	f(".cyberpoverlay .cyberpback", {
		position : "absolute",
		"background-size" : "100% 100%"
	});
	d(".cyberpoverlay", "opacity .15s, visibility .15s, left .01s linear")
})(cyberplayer);
(function (g) {
	var h = cyberplayer.utils;
	g.player = function (f) {
		...
		b._model = c;
		cyberplayer.utils.css.block();
		b.cbPlay = e.play;
		b.cbPause = e.pause;
		b.cbStop = e.stop;
		b.cbSeek = e.seek;
		b.cbSetVolume = e.setVolume;
		b.cbSetMute = e.setMute;
		b.cbLoad = e.load;
		b.cbPlaylistNext = e.next;
		b.cbPlaylistPrev = e.prev;
		b.cbPlaylistItem = e.item;
		b.cbSetFullscreen = e.setFullscreen;
		b.cbResize = k.resize;
		b.cbSeekDrag = c.seekDrag;
		b.cbSetStretching = e.setStretching;
		b.cbGetQualityLevels = e.getQualityLevels;
		b.cbGetCurrentQuality = e.getCurrentQuality;
		b.cbSetCurrentQuality = e.setCurrentQuality;
		b.cbGetCaptionsList = e.getCaptionsList;
		b.cbGetCurrentCaptions = e.getCurrentCaptions;
		b.cbSetCurrentCaptions = e.setCurrentCaptions;
		b.cbSetControls = k.setControls;
		b.cbGetSafeRegion = k.getSafeRegion;
		b.cbForceState = k.forceState;
		b.cbReleaseState = k.releaseState;
		b.cbGetPlaylistIndex = a("item");
		b.cbGetPosition = a("position");
		b.cbGetDuration = a("duration");
		b.cbGetBuffer = a("buffer");
		b.cbGetWidth = a("width");
		b.cbGetHeight = a("height");
		b.cbGetFullscreen = a("fullscreen");
		b.cbGetVolume = a("volume");
		b.cbGetMute = a("mute");
		b.cbGetState = a("state");
		b.cbGetStretching = a("stretching");
		b.cbGetPlaylist = function () {
			for (var a = c.playlist, b = [], e = 0; e < a.length; e++)
				b.push(d(a[e]));
			return b
		};
		b.cbGetControls = a("controls");
		b.cbDetachMedia = e.detachMedia;
		b.cbAttachMedia = e.attachMedia;
		b.cbLoadInstream = function (a, d) {
		...
		b.cbInstreamPlay = function () {
			m && m.cbInstreamPlay()
		};
		b.cbInstreamPause = function () {
			m && m.cbInstreamPause()
		};
		b.cbInstreamDestroy = function () {
			m && m.cbInstreamDestroy();
			m = void 0
		};
		b.cbPlayerDestroy = function () {
			k && k.destroy()
		};
		b.cbAddEventListener = e.addEventListener;
		b.cbRemoveEventListener = e.removeEventListener;
		b.cbDockAddButton = k.addButton;
		b.cbDockRemoveButton = k.removeButton;
		f = new g.setup(c, k, e);
		f.addEventListener(cyberplayer.events.CBPLAYER_READY, function (a) {
			e.playerReady(a);
			h.css.unblock()
		});
		f.addEventListener(cyberplayer.events.CBPLAYER_ERROR,
			function (a) {
...
(function (g) {
	...
	g.playlistcomponent = function (c, k) {
		...
		var j = c,
		r = j.skin,
		q = d.extend({}, h, j.skin.getComponentSettings("playlist"), k),
		n,
		t,
		y,
		p,
		...
		n = m("div", "cyberpplaylist");
		n.id = j.id + "_cyberplayer_playlistcomponent";
		t = m("div", "cyberplistcontainer");
		n.appendChild(t);
		for (var v in x)
			x[v] = r.getSkinElement("playlist",
					v);
		...
		x.itemImage ? (r = (u - x.itemImage.height) / 2 + "px ", v = x.itemImage.width, C = x.itemImage.height) : (v = 4 * u / 3, C = u);
		x.divider && a(e("cyberpplaylistdivider"), {
			"background-image" : "url(" + x.divider.src + ")",
			"background-size" : "100% " + x.divider.height +
			"px",
			width : "100%",
			height : x.divider.height
		});
		a(e("cyberpplaylistimg"), {
			height : C,
			width : v,
		...
		a(e("cyberplist li.active"), v);
		a(e("cyberplist li.active .cyberptitle"), {
		...
		a(e("cyberplist li:hover"), v);
		a(e("cyberplist li:hover .cyberptitle"), {
			color : q.titleovercolor
		});
		a(e("cyberptextwrapper"), {
			height : u - 5,
			position : "relative"
		});
		a(e("cyberptitle"), {
		...
		j.cbAddEventListener(f.CBPLAYER_PLAYLIST_LOADED, function () {
			t.innerHTML = "";
			for (var b = j.cbGetPlaylist(), c = [], e = 0; e < b.length; e++)
			...
			if (y = c) {
				...
				for (b = 0; b < y.length; b++) {
					...
					h = void 0;
					e.id = p.id + "_item_" + f;
					0 < f && (h = m("div", "cyberpplaylistdivider"), e.appendChild(h));
					f = m("div", "cyberpplaylistimg cyberpfill");
					if (c.image || c["playlist.image"] || x.itemImage)
						h =
							void 0, c["playlist.image"] ? h = c["playlist.image"] : c.image ? h = c.image : x.itemImage && (h = x.itemImage.src), a("#" + e.id + " .cyberpplaylistimg", {
							"background-image" : h ? "url(" + h + ")" : null
						}), e.appendChild(f);
					f = m("div", "cyberptextwrapper");
					h = m("span", "cyberptitle");
					h.innerHTML = c && c.title ? c.title : "";
					f.appendChild(h);
					c.description && (h = m("span", "cyberpdescription"), h.innerHTML = c.description, f.appendChild(h));
					e.appendChild(f);
					c = e;
					c.onclick = l(b);
					p.appendChild(c)
				}
				z = j.cbGetPlaylistIndex();
				d.isIOS() && window.iScroll ? (n.innerHTML = "", n.appendChild(p),
					p.style.height = u * y.length + "px", new iScroll(n.id)) : (t.appendChild(p), s = new g.playlistslider(n.id + "_slider", j.skin, n, p))
			}
		});
		j.cbAddEventListener(f.CBPLAYER_PLAYLIST_ITEM, function (a) {
			0 <= z && (b.getElementById(p.id + "_item_" + z).className = "cyberpitem", z = a.index);
			b.getElementById(p.id + "_item_" + a.index).className = "cyberpitem active";
			a = j.cbGetPlaylistIndex();
			a != w && (w = -1, d.isIOS() && window.iScroll ? p.scrollTop = a * u : s && s.visible() && s.thumbPosition(a / (j.cbGetPlaylist().length - 1)))
		});
		return this
	};
	a(".cyberpplaylist", {
	...
	d.dragStyle(".cyberpplaylist", "none");
	a(".cyberpplaylist .cyberpplaylistimg", {
		position : "relative",
		width : "100%",
		"float" : "left",
		margin : "0 5px 0 0",
	...
	a(".cyberpplaylist .cyberplistcontainer", {
		position : "absolute",
		overflow : "hidden",
		width : "100%",
		height : "100%"
	});
	a(".cyberpplaylist .cyberplist li", {
		width : "100%"
	});
	a(".cyberpplaylist .cyberptextwrapper", {
		overflow : "hidden"
	});
	a(".cyberpplaylist .cyberpplaylistdivider", {
		position : "absolute"
	})
})(cyberplayer.html5);
(function (g) {
				...
				if ("rss" != g.parsers.localName(b))
					...
					if (a = l) {
						...
						for (j = 0; j < a.length; j++)
							if (r = a[j], (q = f.filterSources(r.sources)) && q.length)
								r.sources = q, b.push(r);
						l = b
					} else
						l = void 0;
					l && l.length && l[0].sources && l[0].sources.length && l[0].sources[0].file ? k.sendEvent(d.CBPLAYER_PLAYLIST_LOADED, {
						playlist : l
...
(function (g) {
	function h() {
		...
		for (b = 0; b < arguments.length; b++)
			a.push(".cyberpplaylist ." + arguments[b]);
		return a.join(",")
	}
	var f = cyberplayer.utils,
	d = f.css,
	a = document,
	b = window,
	c = void 0;
	g.playlistslider = function (g, e, h, l) {
		...
		function q(a) {
			return (a = z.getSkinElement("playlist",
						a)) ? a : {
				width : 0,
				height : 0,
				src : c
		...
		function n(a) {
			if (A)
				return a = a ? a : b.event, N(v - (a.detail ? -1 * a.detail : a.wheelDelta / 40) / 10), a.stopPropagation && a.stopPropagation(), a.preventDefault && a.preventDefault(), a.cancelBubble = !0, a.cancel = !0, a.returnValue = !1
		}
		function t(a) {
			if (x || "click" == a.type) {
				var b = f.bounds(s),
		...
		function y(a) {
			return function (b) {
				0 < b.button || (N(v + 0.05 * a), C = setTimeout(function () {
		...
		E = r("cyberpslidertop", E, w);
		I = r("cyberpsliderbottom", I, w);
		s = r("cyberprail", null, w);
		u = r("cyberpthumb", null, w);
		E.addEventListener("mousedown", y(-1), !1);
		I.addEventListener("mousedown", y(1), !1);
		r("cyberprailtop",
			g, s);
		r("cyberprailback", B, s, !0);
		r("cyberprailbottom", e, s);
		d(j("cyberprailback"), {
			top : g.height,
			bottom : e.height
		});
		r("cyberpthumbtop", J, u);
		r("cyberpthumbback", h, u, !0);
		r("cyberpthumbbottom", O, u);
		d(j("cyberpthumbback"), {
			top : J.height,
			bottom : O.height
		});
		V();
		l && (l.addEventListener("mousewheel", n, !1), l.addEventListener("DOMMouseScroll", n, !1));
		return this
	};
	d(h("cyberpslider"), {
		position : "absolute",
	...
	d(h("cyberpslidertop", "cyberprailtop", "cyberpthumbtop"), {
		top : 0
	});
	d(h("cyberpsliderbottom", "cyberprailbottom", "cyberpthumbbottom"), {
		bottom : 0
	})
})(cyberplayer.html5);
(function (g) {
	...
	g.rightclick = function (b, c) {
		...
		var m,
		l = h.extend({
				aboutlink : "http://www.baidu.com/cbpabout/?a\x3dr\x26v\x3d" + g.version + "\x26m\x3dh\x26e\x3do",
				abouttext : "关于CyberPlayer " + g.version + "..."
			}, c),
		j = !1,
		r,
		q;
	...
	f(".cyberpclick_item:hover", {
		"background-color" : "#595959",
		color : "#FFF"
	}, !0);
	f(".cyberpclick_item a", {
		"text-decoration" : a,
		color : "#000"
	}, !0);
	f(".cyberpclick hr", {
...
(function (g) {
	...
	g.setup = function (e, h) {
		...
		u = {},
		x,
		v = new d.eventdispatcher,
		C = !1,
		D = [];
		f.extend(this, v);
		this.start = j;
		l(1, function () {
			e.edition && "invalid" == e.edition() ? z("Error setting up player: Invalid license key") : u[1] = !0
		});
		l(b, function () {
			x = new g.skin;
		...
		l(c, function () {
			...
			case "string":
				...
				b.addEventListener(d.CBPLAYER_PLAYLIST_LOADED, n);
				b.addEventListener(d.CBPLAYER_ERROR, y);
		...
		this.getSkinElement = function (d, a) {
			...
			if (f)
				try {
					...
					cyberplayer.utils.log("No such skin component / element: ", [d, a])
				}
			return null
		};
...
(function (g) {
	...
	g.skinloader = function (d, a, b) {
		...
		function e(a) {
			...
			(!b || parseFloat(b) > parseFloat(cyberplayer.version)) && n("Incompatible player version");
			if (0 === a.length)
				q(r);
			else
				for (b = 0; b < a.length; b++) {
					var c =
						j(a[b].getAttribute("name")),
					d = {
						settings : {},
						elements : {},
						layout : {}
		...
		function l(a, b) {
			...
			if (0 !== e.indexOf("data:image/png;base64,"))
				var f = h.getAbsolutePath(p), e = [f.substr(0, f.lastIndexOf("/")), b, e].join("/");
			r[b].elements[d] = {
				height : 0,
				width : 0,
				src : "",
			...
			c.onload = function () {
				...
				e ? (e.height = c.height, e.width = c.width, e.src = c.src, e.ready = !0, m()) : h.log("Loaded an image for a missing element: " + a + "." + d)
			};
			c.onerror =
			function () {
				z = !0;
	...
	g.thumbs = function (a) {
		...
		this.updateTimeline = function (a) {
			...
			if (m) {
				...
				b == m.length && b--;
				if (m[b].text)
					if (a = m[b].text, 0 > a.indexOf("://") && (a = l ? l + "/" + a : a), 0 < a.indexOf("#xywh"))
						try {
						...
						f = new Image, f.addEventListener("load",
							k, !1), f.src = a
			}
		};
...
(function (g) {
	...
	g.html5.video = function (a) {
		...
		function g() {
			F && (E == d.PLAYING && !A) && (x = Number(s.currentTime.toFixed(1)), b(f.CBPLAYER_MEDIA_TIME, {
					position : x,
					duration : u
				}))
		}
		function e(a) {
			F && (v || (v = !0, C || (C = !0, b(f.CBPLAYER_MEDIA_BUFFER_FULL))), "loadedmetadata" == a.type && (s.muted && (s.muted = !1, s.muted = !0), b(f.CBPLAYER_MEDIA_META, {
						duration : s.duration,
		...
		function m(a) {
			F && !A && (s.paused ? s.currentTime ==
				s.duration && 3 < s.duration ? t() : $() : (!h.isFF() || !("play" == a.type && E == d.BUFFERING)) && q(d.PLAYING))
		}
		function l() {
			F && (A || q(d.BUFFERING))
		}
		...
		function r() {
			...
			if ((h.isIPod() || h.isAndroid(2.3)) && !C)
				C = !0, b(f.CBPLAYER_MEDIA_BUFFER_FULL)
		}
		function q(a) {
			if (!(a == d.PAUSED && E == d.IDLE) &&
				!A && E != a) {
				var c = E;
				E = a;
				b(f.CBPLAYER_PLAYER_STATE, {
					oldstate : c,
					newstate : a
				})
			}
		...
		function n() {
			if (F) {
				...
				a != J && (J = a, b(f.CBPLAYER_MEDIA_BUFFER, {
		...
		function t() {
			E != d.IDLE && (U = -1, b(f.CBPLAYER_MEDIA_BEFORECOMPLETE), ca = !0, F && (q(d.IDLE), b(f.CBPLAYER_MEDIA_COMPLETE), ca = !1))
		}
		var y = h.isIE(),
		p = {
			abort : c,
			...
			error : function () {
				F && (h.log("Error playing media: %o", s.error), O.sendEvent(f.CBPLAYER_MEDIA_ERROR, {
			...
			volumechange : function () {
				b(f.CBPLAYER_MEDIA_VOLUME, {
					volume : Math.round(100 * s.volume)
				});
				b(f.CBPLAYER_MEDIA_MUTE, {
					mute : s.muted
				})
			},
			waiting : l
		...
		N.load = function (a) {
			if (F) {
				...
				(a = j(M)) && O.sendEvent(f.CBPLAYER_MEDIA_LEVELS, {
					levels : a,
					currentQuality : U
				});
				r()
		...
		N.stop = function () {
			F && (s.removeAttribute("src"), y || s.load(), U = -1, clearInterval(B), q(d.IDLE))
		};
		N.play = function () {
			F && !A && s.play()
		};
		var $ = N.pause = function () {
			F && (s.pause(), q(d.PAUSED))
		};
		N.seekDrag = function (a) {
			F && ((A = a) ? s.pause() : s.play())
		};
		var ka = N.seek = function (a) {
			F && (v ? (D = 0, s.currentTime = a) : D = a)
		},
		Ma = N.volume = function (a) {
			h.exists(a) && (s.volume = Math.min(Math.max(0, a / 100), 1), I = 100 *
					s.volume)
		};
		N.mute = function (a) {
			h.exists(a) || (a = !s.muted);
			a ? (I = 100 * s.volume, s.muted = !0) : (Ma(I), s.muted = !1)
		};
		...
		N.attachMedia = function (a) {
			F = !0;
			a || (v = !1);
			ca && (q(d.IDLE), b(f.CBPLAYER_MEDIA_COMPLETE), ca = !1)
		};
		N.getTag = function () {
			return s
		};
		...
		N.setCurrentQuality = function (a) {
			U != a && (a = parseInt(a), 0 <= a && (M && M.length > a) && (U = a, h.saveCookie("qualityLabel", M[a].label), b(f.CBPLAYER_MEDIA_LEVEL_CHANGED, {
						currentQuality : a,
						levels : j(M)
					}), a = s.currentTime, r(), N.seek(a)))
		};
...
(function (g) {
	...
	g.view = function (w, s) {
		function u(a) {
			a && (a.element().addEventListener("mousemove", C, t), a.element().addEventListener("mouseout", D, t))
		}
		function x(a, b) {
		...
		function v() {
			clearTimeout(ma);
			if (R.cbGetState() == d.PLAYING || R.cbGetState() ==
				d.PAUSED)
		...
		function A() {
			R.cbGetState() != d.BUFFERING && (Q && (!P && !Y) && Q.hide(), H && (!oa && !Y) && H.hide(), V());
			clearTimeout(ma);
			ma = 0
		}
		function E(a) {
		...
		function I(b, c) {
			h.exists(b) && h.exists(c) && (L.width = b, L.height = c);
			W.style.width = isNaN(b) ? b : b + "px";
			W.style.height = isNaN(c) ? c : c + "px";
			T && T.redraw();
			Q && Q.redraw();
			G && (G.offset(Q && 0 <= G.position().indexOf("bottom") ? Q.height() + Q.margin() : 0), setTimeout(function () {
					H &&
					H.offset("top-left" == G.position() ? G.element().clientWidth + G.margin() : 0)
				}, 500));
			...
			B(c);
			J();
			ra.sendEvent(f.CBPLAYER_RESIZE)
		}
		function B(a) {
			P = (!b || Y) && 40 >= a && 0 > a.toString().indexOf("%");
			Q && (P ? (Q.audioMode(n), $(), T.hidePreview(n), ca(), ka(t)) : (Q.audioMode(t), Oa(R.cbGetState())));
			G && P && V();
			W.style.backgroundColor = P ? "transparent" :
				"#000"
		}
		function J() {
		...
		function F(a) {
			m || (a ? (W.className += " cyberpfullscreen", l.getElementsByTagName("body")[0].style["overflow-y"] = y) : (W.className = W.className.replace(/\s+cyberpfullscreen/, ""), l.getElementsByTagName("body")[0].style["overflow-y"] = ""))
		}
		function M() {
			var a;
			a : {
		...
		function N() {
			T && L.controls && !P && (!k || R.cbGetState() == d.IDLE) && T.show();
			if (b && !Y && (e && (da.style.display = z), !b || !L.fullscreen))
				X.controls = !1
		}
		function ca() {
			T && (b && !Y && (e && L.controls && (da.style.display = p), X.controls = L.controls), T.hide())
		}
		function $() {
			if (L.controls || P)
				Q && Q.show(), U();
		...
		function Oa(a) {
			switch (a) {
			case d.PLAYING:
				!L.getVideo().audioMode() || b ? (ka(n), J(), T.hidePreview(n), b && (!c || !Y) && ca()) : (ka(t), T.hidePreview(P));
				v();
				break;
			case d.IDLE:
				ka(t);
				A();
				P || (T.hidePreview(t), N(), ba.hide || G && !P && G.show());
				break;
			case d.BUFFERING:
				N();
				b ? ka(n) : $();
		...
		H,
		G,
		ba = h.extend({}, L.componentConfig("logo")),
		aa,
		ya,
		P,
		Y = L.mobilecontrols,
		...
		this.setup = function (a) {
			if (!K) {
				...
				X = L.getVideo().getTag();
				fa.appendChild(X);
				da = x("span", "cyberpcontrols");
				Ca = x("span", "cyberpinstream");
				xa = x("span", q);
				a = L.height;
				...
				aa.addEventListener(f.CBPLAYER_CAPTIONS_LIST, E);
				aa.addEventListener(f.CBPLAYER_CAPTIONS_CHANGED, E);
				da.appendChild(aa.element());
				T = new g.display(R, e);
				T.addEventListener(f.CBPLAYER_DISPLAY_CLICK, E);
				P && T.hidePreview(n);
				da.appendChild(T.element());
				G = new g.logo(R, ba);
				da.appendChild(G.element());
				H = new g.dock(R,
						L.componentConfig("dock"));
				da.appendChild(H.element());
				S = R.edition ? new g.rightclick(R, {
						abouttext : L.abouttext,
						aboutlink : L.aboutlink
					}) : new g.rightclick(R, {});
				L.playlistsize && (L.playlistposition && L.playlistposition != p) && (ya = new g.playlistcomponent(R, {}), xa.appendChild(ya.element()));
				if (!b || Y)
					Q = new g.controlbar(R, c), da.appendChild(Q.element()), Y && $();
				setTimeout(function () {
					I(L.width, L.height)
				}, 0);
				wa.appendChild(fa);
				...
				R.cbAddEventListener(f.CBPLAYER_PLAYER_READY, Ea);
				R.cbAddEventListener(f.CBPLAYER_PLAYER_STATE, Na);
				R.cbAddEventListener(f.CBPLAYER_PLAYLIST_COMPLETE, Ma);
				Na({
					newstate : d.IDLE
				});
				...
				h.isIE() && (fa.addEventListener("mousemove", v, t), fa.addEventListener("click", T.clickHandler));
				u(Q);
				u(H);
				u(G)
			}
		...
		var la = this.fullscreen = function (a) {
			...
			a ? L.fullscreen || (F(n), W.requestFullScreen ? W.requestFullScreen() : W.mozRequestFullScreen ? W.mozRequestFullScreen() : W.webkitRequestFullScreen && W.webkitRequestFullScreen(), L.setFullscreen(n)) : (F(t), L.fullscreen && (L.setFullscreen(t), l.cancelFullScreen ?
					l.cancelFullScreen() : l.mozCancelFullScreen ? l.mozCancelFullScreen() : l.webkitCancelFullScreen ? l.webkitCancelFullScreen() : X.webkitExitFullScreen && X.webkitExitFullScreen()), c && R.cbGetState() == d.PAUSED && setTimeout(N, 500));
			Q && Q.redraw();
			T && T.redraw();
			H && H.redraw();
			J();
		...
		this.setupInstream = function (a, b) {
			ua(pa("cyberpinstream"), n);
			ua(pa("cyberpcontrols"), t);
			Ca.appendChild(a);
			_instreamVideo = b;
			Na({
				newstate : d.PLAYING
		...
		this.destroyInstream = function () {
			ua(pa("cyberpinstream"), t);
			ua(pa("cyberpcontrols"), n);
			Ca.innerHTML = "";
			_instreamVideo = null
		};
		this.setupError = function (a) {
			K = !0;
			cyberplayer.embed.errorScreen(W, a);
			Ra()
		};
		...
		this.setControls = function (a) {
			...
			a =
				a ? n : t;
			L.controls = a;
			a != b && (a ? N() : (Q && (!P && !Y) && Q.hide(), H && (!oa && !Y) && H.hide(), V(), ca()), ra.sendEvent(f.CBPLAYER_CONTROLS, {
					controls : a
				}))
		};
		this.forceState = function (a) {
	...
	a(".cyberplayer.cyberpfullscreen", {
		width : "100%",
		height : "100%",
		left : 0,
		right : 0,
	...
	a(".cyberplayer.cyberpfullscreen ." + j, {
		left : 0,
		right : 0,
		top : 0,
		bottom : 0
	...
	a(".cyberplayer .cyberpuniform", {
		"background-size" : "contain !important"
	});
	a(".cyberplayer .cyberpfill", {
		"background-size" : "cover !important",
		"background-position" : "center"
	});
	a(".cyberplayer .cyberpexactfit", {
		"background-size" : "100% 100% !important"
	})
})(cyberplayer.html5);
(function (g) {
	var h = cyberplayer.utils.extend,
	f = g.logo,
	d;
	d = function (a, b) {
		"free" == a.edition() ? b = null : (f.defaults.file = "", f.defaults.prefix = "");
		h(this, new f(a, b))
	};
	d.defaults = {
		prefix : "",
...
(function (g) {
	...
	g.model = function (f, d) {
		var a = new cyberplayer.utils.key(f.key),
		b = new h(f, d),
		c = b.componentConfig;
		b.edition = function () {
			return a.edition()
...
(function (g) {
	...
	g.rightclick = function (d, a) {
		if ("free" == d.edition())
			a.aboutlink = "http://www.baidu.com/cbpabout/?a\x3dr\x26v\x3d" + g.version + "\x26m\x3dh\x26e\x3df", delete a.abouttext;
		else {
			if (!a.aboutlink) {
				var b = "http://www.baidu.com/cbpabout/?a\x3dr\x26v\x3d" + g.version + "\x26m\x3dh\x26e\x3d",
				c = d.edition();
			...
			a.abouttext ? a.abouttext = "About " + a.abouttext + " ..." : (b = d.edition(), b = b.charAt(0).toUpperCase() +
						b.substr(1), a.abouttext = "关于CyberPlayer " + g.version + " (" + b + " edition)")
		}
		h(this, new f(d, a))
	}
})(cyberplayer.html5);
